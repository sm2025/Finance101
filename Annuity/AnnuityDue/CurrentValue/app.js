...
function calculateCV(intrate = interest.value){
    let a = cashflow.value * (1-1/(Math.pow(1+intrate/100,npayments.value)))/(intrate/100)*(1+intrate/100);
    return a; 
}
function calculatenPayments(){
    let a = Math.log(1/(1- (cv.value / cashflow.value / (1+interest.value/100) * interest.value/100)))/Math.log(1+ interest.value/100);
    return a;
}

function calculateCashflow(){
    return cv.value/((1-1/(Math.pow(1+interest.value/100,npayments.value)))/(interest.value/100))/(1+intrate/100);
}
