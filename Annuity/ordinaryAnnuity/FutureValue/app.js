...
function calculateFV(intrate = interest.value){
    let a = cashflow.value * (Math.pow(1+intrate/100,npayments.value)-1)/(intrate/100);
    return a; 
}
function calculatenPayments(){
    let a = Math.log((fv.value/cashflow.value*interest.value/100) +1)/Math.log(1+interest.value/100) ;
    return a;
}

function calculateCashflow(){
    return fv.value/((Math.pow(1+interest.value/100,npayments.value)-1)/(interest.value/100));
}
